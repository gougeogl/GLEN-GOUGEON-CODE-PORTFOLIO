...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		size_t aliasSize = 10 * sizeof(char);
		char alias[aliasSize]; memset(alias, '\0', aliasSize);
...
void getFlipAlias(char* longName, char* alias)
{
	if (strstr(longName, "flip1") != NULL){ strcpy(alias, "flip1"); }
	else if (strstr(longName, "flip2") != NULL){ strcpy(alias, "flip2"); }
	else if (strstr(longName, "flip3") != NULL){ strcpy(alias, "flip3"); }
	else if(strstr(longName, "local") != NULL){ strcpy(alias, "localhost"); }
	else { strcpy(alias, "non-flip host"); }
}
